...
export function Payroll({ employees, timeEntries, onUpdateTimeEntry, onAddTimeEntry }: PayrollProps) {
  ...
  const { language } = useLanguage(); // Ensure instant re-render on language toggle
  ...
  const [lunchDeductionSteps, setLunchDeductionSteps] = useState(0);
  ...
  const pendingAutoFillWhenEntriesLoadRef = useRef(false);
  const prevPayPeriodStableKeyRef = useRef<string | null>(null);
  const prevTimesheetViewForSelectionRef = useRef<'payable' | 'voided' | null>(null);

  const roundToQuarterHours = (hours: number) => Math.round(hours * 4) / 4;
  const payrollHoursForEntry = (entry: Pick<TimeEntry, 'clock_in' | 'clock_out' | 'lunch_deduction_hours'>) => {
    if (!entry.clock_out) return 0;
    const raw = roundToQuarterHours(
      differenceInMinutes(new Date(entry.clock_out), new Date(entry.clock_in)) / 60,
    );
    const deduction = Math.max(0, Number(entry.lunch_deduction_hours ?? 0));
    return Math.max(0, roundToQuarterHours(raw - deduction));
  };
  ...
  const openEditForEntry = (entry: TimeEntry) => {
    ...
    setLunchDeductionSteps(Math.max(0, Math.round(Number(entry.lunch_deduction_hours ?? 0) * 2)));
  };

  useEffect(() => {
    staffSummaryHydratedRef.current = false;
    pendingAutoFillWhenEntriesLoadRef.current = false;
    prevPayPeriodStableKeyRef.current = null;
    prevTimesheetViewForSelectionRef.current = null;
  ...
  const sortedEmployeeIdsWithRelevantEntries = useMemo(() => {
    const periodStart = startOfDay(payPeriodStart);
    const periodEnd = startOfDay(payPeriodEnd);
    const withEntryIds = new Set<string>();
    for (const entry of timeEntries) {
      ...
      if (matches) withEntryIds.add(entry.staff_id);
    }
    return employeesSortedAlphabetically.filter((e) => withEntryIds.has(e.id)).map((e) => e.id);
  }, [timeEntries, employeesSortedAlphabetically, payPeriodStart, payPeriodEnd, timesheetRecordView]);

  useEffect(() => {
    if (!businessId || employees.length === 0 || settingsLoading || staffSummaryHydratedRef.current) return;
    ...
    if (raw == null) {
      next =
        sortedEmployeeIdsWithRelevantEntries.length > 0
          ? sortedEmployeeIdsWithRelevantEntries
          : [];
      pendingAutoFillWhenEntriesLoadRef.current = next.length === 0;
    } else {
      try {
  ...
  }, [businessId, employees, settingsLoading, sortedEmployeeIdsWithRelevantEntries]);

  useEffect(() => {
    if (!pendingAutoFillWhenEntriesLoadRef.current || !staffSummaryHydratedRef.current || settingsLoading) return;
    if (sortedEmployeeIdsWithRelevantEntries.length === 0) return;
    pendingAutoFillWhenEntriesLoadRef.current = false;
    setSelectedEmployeeIds(sortedEmployeeIdsWithRelevantEntries);
  }, [settingsLoading, sortedEmployeeIdsWithRelevantEntries]);

  useEffect(() => {
    if (settingsLoading || !staffSummaryHydratedRef.current) return;
    const key = `${payPeriodStart.toISOString()}|${payPeriodEnd.toISOString()}`;
    if (prevPayPeriodStableKeyRef.current === null) {
      prevPayPeriodStableKeyRef.current = key;
      return;
    }
    if (prevPayPeriodStableKeyRef.current !== key) {
      prevPayPeriodStableKeyRef.current = key;
      pendingAutoFillWhenEntriesLoadRef.current = false;
      setSelectedEmployeeIds(sortedEmployeeIdsWithRelevantEntries);
    }
  }, [settingsLoading, payPeriodStart, payPeriodEnd, sortedEmployeeIdsWithRelevantEntries]);

  useEffect(() => {
    if (!staffSummaryHydratedRef.current) return;
    if (prevTimesheetViewForSelectionRef.current === null) {
      prevTimesheetViewForSelectionRef.current = timesheetRecordView;
      return;
    }
    if (prevTimesheetViewForSelectionRef.current === timesheetRecordView) return;
    prevTimesheetViewForSelectionRef.current = timesheetRecordView;
    pendingAutoFillWhenEntriesLoadRef.current = false;
    setSelectedEmployeeIds(sortedEmployeeIdsWithRelevantEntries);
  }, [timesheetRecordView, sortedEmployeeIdsWithRelevantEntries]);
  ...
  const allPayPeriodEntries = useMemo(() => {
    return timeEntries.filter(entry => {
      ...
      const hours = payrollHoursForEntry(entry);
      return {
        ...entry,
        employee,
        hours,
      ...
      const nameCompare = (a.employee?.name || '').localeCompare(b.employee?.name || '', undefined, {
        sensitivity: 'base',
      });
    ...
    return [...selectedEmployeeIds]
      .map((id) => {
        ...
        const entries: MergedEntry[] = timeEntries
          ...
          .map((entry) => {
            const hours = entry.clock_out ? payrollHoursForEntry(entry) : 0;
            return {
              ...entry,
              employee,
              hours,
  ...
  const selectAllStaffInFilter = () => {
    pendingAutoFillWhenEntriesLoadRef.current = false;
    setSelectedEmployeeIds(employeesSortedAlphabetically.map((e) => e.id));
  };
  ...
  const beginAddEntryForEmployee = (employeeId: string) => {
    ...
    setLunchDeductionSteps(0);
  };

  const adjustLunchDeduction = (deltaSteps: number) => {
    if (!editFormData.clock_out) return;
    const clockInDate = editFormData.clock_in ? new Date(editFormData.clock_in) : null;
    const clockOutDate = new Date(editFormData.clock_out);
    if (Number.isNaN(clockOutDate.getTime())) return;

    if (!clockInDate || Number.isNaN(clockInDate.getTime())) return;
    const rawHours = Math.max(0, differenceInMinutes(clockOutDate, clockInDate) / 60);
    const maxSteps = Math.max(0, Math.floor(rawHours * 2));
    setLunchDeductionSteps((prev) => Math.min(maxSteps, Math.max(0, prev + deltaSteps)));
  };
  ...
  const employeeTimesheetEntries = useMemo(() => {
    ...
    return payPeriodDays.map(day => {
      ...
      const totalHours = dayEntries.reduce((sum, entry) => {
        ...
        return sum + payrollHoursForEntry(entry);
      }, 0);
  ...
  const handleSaveAmend = async () => {
    ...
    if (!editFormData.clock_in) {
      toast.error(t('payroll.clockInRequired'));
      return;
    }

    const clockInDate = new Date(editFormData.clock_in);
    const clockOutDate = editFormData.clock_out ? new Date(editFormData.clock_out) : null;
    ...
    if (clockOutDate && clockOutDate.getTime() <= clockInDate.getTime()) {
      toast.error(t('payroll.clockOutMustBeAfterClockIn'));
      return;
    }

    const clockInISO = clockInDate.toISOString();
    const clockOutISO = clockOutDate ? clockOutDate.toISOString() : undefined;
    ...
    if (editingEntry) {
      ...
      const updated = await onUpdateTimeEntry(editingEntry.id, {
        ...
        lunch_deduction_hours: lunchDeductionSteps * 0.5,
      });
      ...
      if (created) {
        if (lunchDeductionSteps > 0) {
          await onUpdateTimeEntry(created.id, { lunch_deduction_hours: lunchDeductionSteps * 0.5 });
        }
        setEditingDay(null);
        setEditingEntry(null);
        setEditFormData({ clock_in: '', clock_out: '' });
        ...
        toast.error(t('payroll.addEntryFailed'));
      }
    }
  };
  ...
  const handleDownloadPDF = async () => {
    const formatPdfDate = (date: Date, pattern: string) =>
      format(date, pattern, { locale: dateLocale });

    const doc = new jsPDF();
    const businessName = settings.business_name || 'Business';
    const fileBaseName = `${businessName} - ${t('payroll.reportFileLabel')} - ${format(payPeriodEnd, 'yyyy-MM-dd')}`;
    const safeFileName = `${fileBaseName.replace(/[<>:"/\\|?*\u0000-\u001F]/g, '').replace(/\s+/g, ' ').trim()}.pdf`;
    ...
    const headline = `${businessName} - ${t('payroll.title').toUpperCase()}`;
    ...
    doc.text(
      `${t('payroll.payPeriod')}: ${formatPdfDate(payPeriodStart, 'PP')} - ${formatPdfDate(payPeriodEnd, 'PP')}`,
      margin,
      yPos,
    );
    yPos += 6;
    doc.text(`${t('payroll.generatedOn')}: ${formatPdfDate(new Date(), 'PPPP p')}`, margin, yPos);
    yPos += 12;
    ...
    payCalcData.push([t('payroll.totals'), '', '', grandTotalHours.toFixed(2), `$${grandTotalPay.toFixed(2)}`]);

    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(35, 35, 35);
    doc.text(t('payroll.payCalculations').toUpperCase(), margin, yPos);
    yPos += 6;

    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    const payCalcDescriptionLines = doc.splitTextToSize(t('payroll.payCalculationsDescription'), availableWidth);
    payCalcDescriptionLines.forEach((line) => {
      doc.text(line, margin, yPos);
      yPos += 4.5;
    });
    yPos += 5.5;

    const payCalcTotalBg: [number, number, number] = [232, 242, 255];
    const payCalcLineGray: [number, number, number] = [210, 210, 210];
    const payCalcThinRow = { top: 0, left: 0, right: 0, bottom: 0.12 } as const;
    const payCalcNoLine = { top: 0, left: 0, right: 0, bottom: 0 } as const;

    autoTable(doc, {
      head: [[t('payroll.employeeId'), t('payroll.employee'), t('payroll.hourlyRate'), t('payroll.hoursWorked'), t('payroll.totalPay')]],
      body: payCalcData,
      startY: yPos,
      ...
      styles: {
        ...
        lineColor: payCalcLineGray,
        lineWidth: payCalcThinRow,
        ...centeredCell,
      },
      headStyles: {
        ...
        textColor: 255,
        fontStyle: 'bold',
        lineWidth: payCalcNoLine,
        lineColor: payCalcLineGray,
        ...centeredCell,
      },
      columnStyles: {
        0: { cellWidth: availableWidth * 0.15, halign: 'center' },
        1: { cellWidth: availableWidth * 0.3, halign: 'left' },
        2: { cellWidth: availableWidth * 0.18, halign: 'right' },
        3: { cellWidth: availableWidth * 0.18, halign: 'right' },
        4: { cellWidth: availableWidth * 0.19, halign: 'right' },
      },
      didParseCell: (data: any) => {
        ...
        const isTotalRow = data.row.raw[0] === t('payroll.totals');
        if (isTotalRow) {
          data.cell.styles.fontStyle = 'bold';
          data.cell.styles.fillColor = payCalcTotalBg;
          data.cell.styles.textColor = [25, 25, 25];
          data.cell.styles.halign = data.column.index === 1 ? 'left' : data.column.index >= 2 ? 'right' : 'center';
        } else if (data.row.index === payCalcData.length - 2) {
          data.cell.styles.lineWidth = payCalcNoLine;
        }
      },
    });
    ...
    if (staffSummaryBlocks.length > 0) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(35, 35, 35);
      doc.text(t('payroll.employeeSummary').toUpperCase(), margin, yPos);
      yPos += 10;
      ...
      for (const block of staffSummaryBlocks) {
        ...
        doc.setTextColor(35, 35, 35);
        doc.text(
          `${block.employee.name} (${t('payroll.employeeId')}: ${getEmployeeId(block.employee.id)})`,
          margin,
          yPos,
        );
        yPos += 8;
        ...
        const bodyRows: (string | number)[][] = block.rows.map(({ entry, hours }) => [
          formatPdfDate(new Date(entry.clock_in), 'P'),
          formatPdfDate(new Date(entry.clock_in), 'p'),
          entry.clock_out ? formatPdfDate(new Date(entry.clock_out), 'p') : t('payroll.openEntry'),
          entry.clock_out ? Number(entry.lunch_deduction_hours ?? 0).toFixed(1) : '—',
          entry.clock_out ? (Math.round(hours * 4) / 4).toFixed(2) : '—',
        ]);
        const totalLunchDeduction = block.rows.reduce(
          (sum, r) => sum + (r.entry.clock_out ? Number(r.entry.lunch_deduction_hours ?? 0) : 0),
          0,
        );
        bodyRows.push([
        ...
        autoTable(doc, {
          head: [[
            ...
            t('payroll.clockIn'),
            t('payroll.clockOut'),
            t('payroll.lunchDeductionColumn'),
          ...
          columnStyles: {
            0: { cellWidth: availableWidth * 0.23, halign: 'left' },
            1: { cellWidth: availableWidth * 0.2, halign: 'center' },
            2: { cellWidth: availableWidth * 0.2, halign: 'center' },
            3: { cellWidth: availableWidth * 0.17, halign: 'right' },
            4: { cellWidth: availableWidth * 0.2, halign: 'right', fontStyle: 'bold' },
          },
          didParseCell: (data: any) => {
            if (data.section === 'head') {
            ...
            const isTotalRow =
              typeof raw0 === 'string' && raw0.startsWith(`${t('employeePayroll.total')}:`);
            if (isTotalRow) {
              data.cell.styles.fillColor = summarySubtotalBg;
              data.cell.styles.textColor = [25, 25, 25];
              data.cell.styles.fontStyle = 'bold';
              if (data.column.index === 0) data.cell.styles.halign = 'left';
              else if (data.column.index === 3 || data.column.index === 4) data.cell.styles.halign = 'right';
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="text-lg font-semibold sm:text-xl">
            {t('payroll.payPeriod')}: {format(payPeriodStart, 'MMMM d', { locale: dateLocale })} -{' '}
            {format(payPeriodEnd, 'MMMM d, yyyy', { locale: dateLocale })}
          </p>
        </div>
                  ...
                  <Popover open={staffFilterOpen} onOpenChange={setStaffFilterOpen}>
                    ...
                    <PopoverContent className="w-[min(100vw-2rem,22rem)] p-0" align="end">
                      ...
                      <div className="max-h-[min(50vh,16rem)] overflow-y-auto p-2">
                        {employeesSortedAlphabetically.map((emp) => (
                          <label
                            key={emp.id}
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="whitespace-nowrap px-2 py-2.5 text-right text-muted-foreground sm:py-3 sm:pr-2">
                                    {entry.clock_out
                                      ? Number(entry.lunch_deduction_hours ?? 0).toFixed(1)
                                      : '—'}
                                  </td>
                        ...
                        {block.entries.length > 0 && (
                          <tfoot>
                            <tr className="border-t-2 border-border bg-primary/10 font-semibold">
                              ...
                              <td className="py-2.5 pr-2 text-right text-muted-foreground sm:py-3 sm:pr-2">
                                {block.entries
                                  .reduce((sum, e) => sum + Number(e.lunch_deduction_hours ?? 0), 0)
                                  .toFixed(1)}
                              </td>
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      setEditFormData({ ...editFormData, clock_out: e.target.value });
                      setLunchDeductionSteps(0);
                    }}
                  />
                  <div className="flex flex-wrap items-center gap-2">
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      disabled={!editFormData.clock_out || lunchDeductionSteps === 0}
                      onClick={() => adjustLunchDeduction(-1)}
                    >
                      +0.5h
                    </Button>
                    <Button
                      ...
                      variant="secondary"
                      size="sm"
                      disabled={!editFormData.clock_out}
                      onClick={() => adjustLunchDeduction(1)}
                    >
                      -0.5h {t('payroll.lunch')}
                    </Button>
                    {lunchDeductionSteps > 0 ? (
                      <span className="text-xs text-muted-foreground">
                        {t('payroll.lunchDeducted')}: {(lunchDeductionSteps * 0.5).toFixed(1)}h
                      </span>
                    ) : null}
                  </div>
