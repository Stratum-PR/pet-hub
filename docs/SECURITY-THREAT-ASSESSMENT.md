...
| ID | Fix | Where |
|----|-----|--------|
| **H2** | User-facing errors no longer show raw `error.message`. Generic message (e.g. "Something went wrong. Please try again.") used in toasts and error UI. | Login, AuthCallback, Dashboard, BusinessServices, BusinessPets, BusinessSettings, PetForm, AdminDashboard, Register, GlobalErrorBoundary |
...
| Impersonation | [src/pages/AdminDashboard.tsx](src/pages/AdminDashboard.tsx), [src/pages/ImpersonateHandler.tsx](src/pages/ImpersonateHandler.tsx) | Token generated via `supabase.rpc('generate_impersonation_token', { target_business_id })`. Consumption via `supabase.rpc('use_impersonation_token', { impersonation_token })` then sessionStorage set. |
| Support user session (full RLS) | [supabase/functions/support-begin-user-session/index.ts](supabase/functions/support-begin-user-session/index.ts), [src/components/SupportImpersonationDialog.tsx](src/components/SupportImpersonationDialog.tsx), [src/lib/supportSession.ts](src/lib/supportSession.ts) | Super admin calls Edge Function with JWT; server verifies `profiles.is_super_admin`, checks `staff` row + `business_id`, uses Auth Admin `generateLink` to return `hashed_token`; client calls `verifyOtp` so the browser session becomes the **target user** (same data access as that user). Admin refresh/access tokens are stored in `sessionStorage` under `support_admin_session_snapshot` until “Exit support view” restores them via `setSession` or the user signs out (markers cleared in [src/lib/auth.ts](src/lib/auth.ts)). Each issuance appends a row to `public.support_impersonation_audit` (admin_id, target_user_id, business_id, ip, user_agent). |
| Feature preview tier | [src/lib/featureRollout.ts](src/lib/featureRollout.ts), [src/hooks/useFeatureRollout.ts](src/hooks/useFeatureRollout.ts), migrations `20260331170000_feature_rollout_geofence_tier.sql` | Super admins only: UI tier (Dev/Staged/Prod) in sessionStorage; RPCs such as `clock_in_out` accept optional `p_support_feature_tier` which the database **ignores** unless `auth.uid()` is a super admin (`resolve_support_feature_viewer_tier`). Non-admins always evaluate as production for gated features. |
...
| Edge Functions | [supabase/functions/rate-limited-reset-password/index.ts](supabase/functions/rate-limited-reset-password/index.ts), [supabase/functions/support-begin-user-session/index.ts](supabase/functions/support-begin-user-session/index.ts) | Password reset: rate limit 3/email/hour via Upstash; CORS uses `ALLOWED_ORIGINS` env (see Pending issues). **Support session:** requires `SUPABASE_SERVICE_ROLE_KEY` and `SUPABASE_ANON_KEY`; validates caller JWT as super admin; CORS should mirror other functions via `ALLOWED_ORIGINS`. |
...
**Support user session (QuickBase-style):**  
- **Exploit:** Compromise device or XSS while support mode is active: attacker could read `support_admin_session_snapshot` in sessionStorage and attempt to restore the admin session, or abuse an open support session as the target user. Mis-issued `hashed_token` if Edge Function were misconfigured could allow unintended login (mitigated by super-admin check + staff/business validation).  
- **Mitigation:** Edge Function verifies super admin in DB; binds `target_staff_id` to `business_id`; writes `support_impersonation_audit` for traceability. Ops should protect admin workstations, use short support sessions, sign out when done, and restrict Edge deployment secrets. Review audit log periodically for anomalies.
